...
export function runAnalyze(root: string, json: boolean): void {
  ...
  if (i18nFramework.name) {
    ...
    console.log(
      `- **Config**: ${i18nFramework.configExists ? i18nFramework.configPath : "not found"}`,
    );
  }
  ...
  if (locales.locales.length > 0) {
    ...
    console.log("1. Installing an i18n library (e.g., `next-intl`, `react-i18next`, `vue-i18n`)");
    console.log("2. Creating a `messages/` or `locales/` directory with locale JSON files");
    console.log("3. Running `i18n-agent add-locale en --mode stub` to create a starter file");
  }

  if (keyAnalysis) {
    console.log("\n## Key Coverage\n");
    console.log(
      `Base locale **${keyAnalysis.baseLocale}** has **${keyAnalysis.totalKeysInBase}** keys.\n`,
    );

    if (Object.keys(keyAnalysis.coverage).length > 0) {
      console.log("| Locale | Total | Matching | Missing | Extra | Coverage |");
      console.log("|--------|-------|----------|---------|-------|----------|");
      for (const cov of Object.values(keyAnalysis.coverage)) {
        console.log(
          `| ${cov.locale} | ${cov.totalKeys} | ${cov.matchingKeys} | ${cov.missingKeys} | ${cov.extraKeys} | ${cov.coveragePercent}% |`,
        );
      }
    }
