...
describe("scanner", () => {
  it("detects static JSX text in visible elements", async () => {
    ...
    const welcomeString = result.candidates.find(
      (c) => c.source === "Welcome to our app",
    );
  ...
  it("detects button text", async () => {
    ...
    const buttonString = result.candidates.find(
      (c) => c.source === "Get started",
    );
  ...
  it("detects alt attributes", async () => {
    ...
    const alt = result.candidates.find(
      (c) => c.propName === "alt" && c.source === "Company logo",
    );
  ...
  it("marks conditional strings as REVIEW_REQUIRED", async () => {
    ...
    const conditionals = result.candidates.filter(
      (c) => c.risk === "REVIEW_REQUIRED",
    );
  ...
  it("infers route for app directory pages", async () => {
    ...
    const settingsString = result.candidates.find(
      (c) => c.source === "Account settings",
    );
  ...
  it("detects component type from use client directive", async () => {
    ...
    const headerStrings = result.candidates.filter(
      (c) => c.filePath.includes("Header"),
    );
